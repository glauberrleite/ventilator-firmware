...
void IRAM_ATTR onTimer()
{
  ...
  {
    if (!ins_pause)
    { /// SE O BOTÃO INSP NÃO TIVER SIDO PRESSIONADO (CONDIÇÃO NORMAL)
    ...
    { /// SE O BOTÃO INSP TIVER SIDO PRESSIONADO
      if (timer_counter >= time_inhale_to_exhale)
      {
...
void loop()
{
  ...
  case INHALE_PCV:
    ...
    pid_out = delta_ins > 20 ? prev_pid_out + 20 : pid_out;
    pid_out = delta_ins < -2 ? prev_pid_out - 2 : pid_out;
  ...
  case INHALE_TO_EXHALE:
    ...
    if (mode == PCV)
    {
      ...
      if (abs(pres_peak - last_ins_pressure) > 1)
